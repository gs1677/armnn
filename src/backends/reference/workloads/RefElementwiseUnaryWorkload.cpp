...
RefElementwiseUnaryWorkload::RefElementwiseUnaryWorkload(
    const ElementwiseUnaryQueueDescriptor &desc, const WorkloadInfo &info)
...
void RefElementwiseUnaryWorkload::ExecuteAsync(
    WorkingMemDescriptor &workingMemDescriptor) {

  Execute(workingMemDescriptor.m_Inputs, workingMemDescriptor.m_Outputs);
}

void RefElementwiseUnaryWorkload::Execute(
    std::vector<ITensorHandle *> inputs,
    std::vector<ITensorHandle *> outputs) const {
  ARMNN_SCOPED_PROFILING_EVENT(Compute::CpuRef,
                               "RefElementwiseUnaryWorkload_Execute");
  ...
  std::unique_ptr<Decoder<InType>> input =
      MakeDecoder<InType>(inputInfo, inputs[0]->Map());
  std::unique_ptr<Encoder<OutType>> output =
      MakeEncoder<OutType>(outputInfo, outputs[0]->Map());
  ...
  using CosFunction = ElementwiseUnaryFunction<cos<InType>>;
  ...
  case UnaryOperation::Cos: {
    CosFunction(inShape, outShape, *input, *output);
    break;
  }
  ...
  default: {
    throw InvalidArgumentException(
        std::string("Unsupported unary operation ") +
            GetUnaryOperationAsCString(m_Data.m_Parameters.m_Operation),
        CHECK_LOCATION());
  }
  }
}
