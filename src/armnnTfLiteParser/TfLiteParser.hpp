...
public:
  ...
  armnn::INetworkPtr
  CreateNetworkFromBinary(const std::vector<uint8_t> &binaryContent);

  /// Retrieve binding info (layer id and tensor info) for the network input
  /// identified by the given layer name and subgraph id
  BindingPointInfo GetNetworkInputBindingInfo(size_t subgraphId,
                                              const std::string &name) const;

  /// Retrieve binding info (layer id and tensor info) for the network output
  /// identified by the given layer name and subgraph id
  ...
  GetSubgraphOutputTensorNames(size_t subgraphId) const;

  TfLiteParserImpl(const armnn::Optional<ITfLiteParser::TfLiteParserOptions>
...
public:
  // testable helpers
  armnn::INetworkPtr
  CreateNetworkFromBinaryAsDynamic(const std::vector<uint8_t> &binaryContent);

  armnn::INetworkPtr LoadModel(std::unique_ptr<tflite::ModelT> model);

  static ModelPtr LoadModelFromFile(const char *fileName);
  static ModelPtr LoadModelFromBinary(const uint8_t *binaryContent, size_t len);
  static TensorRawPtrVector
  GetInputs(const ModelPtr &model, size_t subgraphIndex, size_t operatorIndex);
  static TensorRawPtrVector
  GetOutputs(const ModelPtr &model, size_t subgraphIndex, size_t operatorIndex);
  ...
  static armnn::TensorInfo
  OutputShapeOfSqueeze(std::vector<uint32_t> squeezeDims,
...
private:
  ...
  using OperatorParsingFunction =
      void (TfLiteParserImpl::*)(size_t subgraphIndex, size_t operatorIndex);
  ...
  void ParseActivation(size_t subgraphIndex, size_t operatorIndex,
                       armnn::ActivationFunction activationType);
  void ParseAdd(size_t subgraphIndex, size_t operatorIndex);
  void ParseArgMinMax(size_t subgraphIndex, size_t operatorIndex,
  ...
  void ParseComparison(size_t subgraphIndex, size_t operatorIndex,
...
// define is needed.
  ...
  void ParseElementwiseUnary(size_t subgraphIndex, size_t operatorIndex,
  ...
  void ParseLocalResponseNormalization(size_t subgraphIndex,
                                       size_t operatorIndex);
  ...
  void ParsePool(size_t subgraphIndex, size_t operatorIndex,
                 armnn::PoolingAlgorithm algorithm);
  void ParsePrelu(size_t subgraphIndex, size_t operatorIndex);
  void ParseQuantize(size_t subgraphIndex, size_t operatorIndex);
  void ParseReduce(size_t subgraphIndex, size_t operatorIndex,
  ...
  void ParseResize(size_t subgraphIndex, size_t operatorIndex,
  ...
  void ParseCos(size_t subgraphIndex, size_t operatorIndex);
  ...
  void RegisterProducerOfTensor(size_t subgraphIndex, size_t tensorIndex,
                                armnn::IOutputSlot *slot);
  void RegisterConsumerOfTensor(size_t subgraphIndex, size_t tensorIndex,
                                armnn::IInputSlot *slot);
  void RegisterInputSlots(size_t subgraphIndex, size_t operatorIndex,
                          armnn::IConnectableLayer *layer,
                          const std::vector<unsigned int> &tensorIndexes,
                          unsigned int startingSlotIndex = 0);
  void RegisterOutputSlots(size_t subgraphIndex, size_t operatorIndex,
                           armnn::IConnectableLayer *layer,
                           const std::vector<unsigned int> &tensorIndexes);
  ...
  void AddBroadcastReshapeLayer(size_t subgraphIndex, size_t operatorIndex,
                                armnn::IConnectableLayer *layer);

  /// Attach an activation layer to the one passed as a parameter
  armnn::IConnectableLayer *
  AddFusedActivationLayer(armnn::IConnectableLayer *layer,
                          unsigned int outputSlot,
                          tflite::ActivationFunctionType activationType);

  // SupportedDataStorage's purpose is to hold data till we pass over to the
  // network. We don't care about the content, and we want a single datatype to
  // simplify the code.
  ...
  std::pair<armnn::ConstTensor, SupportedDataStorage> CreateConstTensorPermuted(
  ...
  std::unordered_map<std::string, OperatorParsingFunction>
      m_CustomParserFunctions;

  /// A mapping of an output slot to each of the input slots it should be
  /// connected to The outputSlot is from the layer that creates this tensor as
  /// one of its ouputs The inputSlots are from the layers that use this tensor
