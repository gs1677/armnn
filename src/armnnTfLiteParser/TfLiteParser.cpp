  ...
  {                                                                            \
    throw armnn::ParseException(                                               \
        static_cast<const std::stringstream &>(                                \
            std::stringstream() << msg << ": " << CHECK_LOCATION().AsString()) \
            .str());                                                           \
  }
...
ITfLiteParser::ITfLiteParser(
    const armnn::Optional<TfLiteParserOptions> &options)
    : pTfLiteParserImpl(new TfLiteParserImpl(options)) {}

ITfLiteParser::~ITfLiteParser() = default;

ITfLiteParser *
ITfLiteParser::CreateRaw(const armnn::Optional<TfLiteParserOptions> &options) {
  return new ITfLiteParser(options);
}

ITfLiteParserPtr
ITfLiteParser::Create(const armnn::Optional<TfLiteParserOptions> &options) {
  return ITfLiteParserPtr(CreateRaw(options), &ITfLiteParser::Destroy);
}

void ITfLiteParser::Destroy(ITfLiteParser *parser) { delete parser; }

armnn::INetworkPtr
ITfLiteParser::CreateNetworkFromBinaryFile(const char *graphFile) {
  return pTfLiteParserImpl->CreateNetworkFromBinaryFile(graphFile);
}

armnn::INetworkPtr ITfLiteParser::CreateNetworkFromBinary(
    const std::vector<uint8_t> &binaryContent) {
  return pTfLiteParserImpl->CreateNetworkFromBinary(binaryContent);
}

BindingPointInfo
ITfLiteParser::GetNetworkInputBindingInfo(size_t subgraphId,
                                          const std::string &name) const {
  return pTfLiteParserImpl->GetNetworkInputBindingInfo(subgraphId, name);
}

BindingPointInfo
ITfLiteParser::GetNetworkOutputBindingInfo(size_t subgraphId,
                                           const std::string &name) const {
...
ITfLiteParser::GetSubgraphInputTensorNames(size_t subgraphId) const {
  return pTfLiteParserImpl->GetSubgraphInputTensorNames(subgraphId);
}

std::vector<std::string>
ITfLiteParser::GetSubgraphOutputTensorNames(size_t subgraphId) const {
...
void CheckSubgraph(const TfLiteParserImpl::ModelPtr &model,
                   size_t subgraphIndex, const CheckLocation &location) {
  if (model.get() == nullptr) {
    throw ParseException(fmt::format(
        ...
        "subgraph:{} at {}",
        location.m_Function, subgraphIndex, location.FileLine()));
...
void CheckModel(const TfLiteParserImpl::ModelPtr &model, size_t subgraphIndex,
                size_t operatorIndex, const CheckLocation &location) {
  if (model.get() == nullptr) {
    throw ParseException(fmt::format(
        "{} was called with invalid (null) model. "
        "Possible reason is that the model is not yet loaded and Unpack(ed). "
        "subgraph:{} operator:{} at {}",
        location.m_Function, subgraphIndex, operatorIndex,
        location.FileLine()));
  } else if (subgraphIndex >= model->subgraphs.size()) {
    throw ParseException(
        fmt::format("{} was called with an invalid subgraph index. "
                 ...
                 model->subgraphs[subgraphIndex]->operators.size() &&
...
void CheckTensor(const TfLiteParserImpl::ModelPtr &model, size_t subgraphIndex,
                 size_t tensorIndex, const CheckLocation &location) {
  // not checking model, because I assume CHECK_MODEL already run
  // and checked that. An assert would do.
  ARMNN_ASSERT_MSG(model.get() != nullptr,
...
void CheckTensorPtr(TfLiteParserImpl::TensorRawPtr rawPtr,
  ...
  if (rawPtr == nullptr) {
    throw ParseException(
        fmt::format("{} was called with a null tensor pointer at {}",
                    location.m_Function, location.FileLine()));
  }
}
...
void CheckBuffer(const TfLiteParserImpl::ModelPtr &model, size_t bufferIndex,
                 const CheckLocation &location) {
  if (model.get() == nullptr) {
    throw ParseException(fmt::format(
        "{} was called with invalid (null) model. "
        "Possible reason is that the model is not yet loaded and Unpack(ed). "
        "buffer:{} at {}",
        location.m_Function, bufferIndex, location.FileLine()));
    ...
    throw ParseException(fmt::format("The buffer #{} is null. {}", bufferIndex,
                                     location.AsString()));
  }
}
...
void CheckBufferSize(TfLiteParserImpl::BufferRawPtr bufferPtr,
                     const armnn::TensorInfo &tensorInfo, uint32_t bufferId,
                     const CheckLocation &location) {
  if (bufferPtr == nullptr) {
    throw ParseException(fmt::format("BufferPtr is null for buffer:{}. {}",
                                     bufferId, location.AsString()));
  } else if (tensorInfo.GetNumElements() > bufferPtr->data.size() ||
             tensorInfo.GetNumBytes() > bufferPtr->data.size()) {
    std::stringstream ss;
    ss << "Buffer #" << bufferId << " has " << bufferPtr->data.size()
       << " bytes. "
...
#define CHECK_SUPPORTED_FUSED_ACTIVATION(OPTION, SUBGRAPH_INDEX,               \
                                         OPERATOR_INDEX)                       \
  do {                                                                         \
      ...
      throw ParseException(                                                    \
          fmt::format("TfLite parser doesn't suppport fused activation: "      \
                      ...
                      tflite::EnumNameActivationFunctionType(                  \
                          OPTION->fused_activation_function),                  \
                      __func__, SUBGRAPH_INDEX, OPERATOR_INDEX,                \
...
void CalcPadding(uint32_t inputSize, uint32_t filterSize, uint32_t stride,
                 ...
                 uint32_t &paddingBack, tflite::Padding padding) {
  paddingFront = 0;
  paddingBack = 0;
...
armnn::TensorInfo ToTensorInfo(TfLiteParserImpl::TensorRawPtr tensorPtr,
  ...
  default: {
    ...
    throw ParseException(fmt::format(
        "Unsupported data type {} = {} for tensor: {}. {}", tensorPtr->type,
        tflite::EnumNameTensorType(tensorPtr->type), tensorPtr->name,
        location.AsString()));
  }
  }
  ...
  if (!outputTensor) {
    tensorShape = TensorShape(
        armnn::numeric_cast<unsigned int>(safeShape.size()), safeShape.data());
  } else {
    ...
    if (shapeSignatureSize != 0) {
      // If the shape is incompatible with the shape signature override the
      // shape
      if (shapeSignatureSize != shape.size()) {
        ...
        for (unsigned int i = 0; i < shapeSignatureSize; ++i) {
          unsigned int dim =
              tensorPtr->shape_signature[i] > -1
                  ? static_cast<unsigned int>(tensorPtr->shape_signature[i])
                  : 0;
          safeShape.push_back(dim);
        }
      }

      std::unique_ptr<bool[]> dimMask =
          std::make_unique<bool[]>(tensorPtr->shape_signature.size());
      for (unsigned int i = 0; i < tensorPtr->shape_signature.size(); ++i) {
        dimMask[i] = tensorPtr->shape_signature[i] == -1 ? false : true;
      }
      tensorShape = TensorShape(static_cast<unsigned int>(safeShape.size()),
                                safeShape.data(), dimMask.get());
    }
    // If there is no shape signature treat the tensor as dynamic if the shape
    // has a size of zero
    else if (shape.size() == 0) {
      tensorShape = TensorShape(1, false);
    } else {
      tensorShape = TensorShape(armnn::numeric_cast<unsigned int>(shape.size()),
                                shape.data());
    }
  }
  ...
  if (tensorPtr->quantization.get()) {
    if (tensorPtr->quantization->scale.size() <= 1) {
      ...
      if (tensorPtr->quantization->zero_point.size() == 1) {
        ...
        quantizationOffset = armnn::numeric_cast<int32_t>(
            tensorPtr->quantization->zero_point[0]);
      }

      armnn::TensorInfo result(tensorShape, type, quantizationScale,
                               quantizationOffset);
      return result;
    } else {
      ...
      // QSymmS8 Per-axis
      armnn::TensorInfo result(
          tensorShape, type, quantizationScales,
...
armnn::LayerBindingId GenerateLayerBindingId(size_t subgraphIndex,
                                             size_t tensorIndex) {
...
void CheckMatchingQuantization(const TensorInfo &first,
  ...
  if (!first.IsQuantized() || !second.IsQuantized()) {
  ...
  if (firstDataType != secondDataType) {
    throw InvalidArgumentException(
        descName + ": " + firstName + " and " + secondName +
        " must be of the same quantized type, " + firstName + " is " +
        GetDataTypeName(firstDataType) + ", " + secondName + " is " +
        GetDataTypeName(secondDataType));
  }

  if (!first.IsTypeSpaceMatch(second)) {
    throw InvalidArgumentException(
        descName + ": " + firstName + " and " + secondName +
        " must have the same quantization space, " + firstName +
        " has offset " + std::to_string(first.GetQuantizationOffset()) +
        " and scale " + std::to_string(first.GetQuantizationScale()) + ", " +
        secondName + " has offset " +
        std::to_string(second.GetQuantizationOffset()) + " and scale " +
        std::to_string(second.GetQuantizationScale()));
  }
}

} // namespace

TfLiteParserImpl::TfLiteParserImpl(
    const Optional<ITfLiteParser::TfLiteParserOptions> &options)
    : m_Options(options), m_Network(nullptr, nullptr),
      m_ParserFunctions(tflite::BuiltinOperator_MAX + 1,
  ...
  m_ParserFunctions[tflite::BuiltinOperator_ARG_MIN] =
      &TfLiteParserImpl::ParseArgMin;
  m_ParserFunctions[tflite::BuiltinOperator_ARG_MAX] =
      &TfLiteParserImpl::ParseArgMax;
  m_ParserFunctions[tflite::BuiltinOperator_AVERAGE_POOL_2D] =
      &TfLiteParserImpl::ParseAveragePool2D;
  m_ParserFunctions[tflite::BuiltinOperator_BATCH_TO_SPACE_ND] =
      &TfLiteParserImpl::ParseBatchToSpaceND;
  m_ParserFunctions[tflite::BuiltinOperator_CAST] =
      &TfLiteParserImpl::ParseCast;
  m_ParserFunctions[tflite::BuiltinOperator_CONCATENATION] =
      &TfLiteParserImpl::ParseConcatenation;
  m_ParserFunctions[tflite::BuiltinOperator_CONV_2D] =
      &TfLiteParserImpl::ParseConv2D;
// Conv3D support was added in TF 2.5, so for backwards compatibility a hash
// define is needed.
#if defined(ARMNN_POST_TFLITE_2_3)
  m_ParserFunctions[tflite::BuiltinOperator_CONV_3D] =
      &TfLiteParserImpl::ParseConv3D;
#endif
  m_ParserFunctions[tflite::BuiltinOperator_CUSTOM] =
      &TfLiteParserImpl::ParseCustomOperator;
  m_ParserFunctions[tflite::BuiltinOperator_DEPTH_TO_SPACE] =
      &TfLiteParserImpl::ParseDepthToSpace;
  m_ParserFunctions[tflite::BuiltinOperator_DEPTHWISE_CONV_2D] =
      &TfLiteParserImpl::ParseDepthwiseConv2D;
  m_ParserFunctions[tflite::BuiltinOperator_DEQUANTIZE] =
      &TfLiteParserImpl::ParseDequantize;
  ...
  m_ParserFunctions[tflite::BuiltinOperator_EQUAL] =
      &TfLiteParserImpl::ParseEqual;
  m_ParserFunctions[tflite::BuiltinOperator_EXP] = &TfLiteParserImpl::ParseExp;
  m_ParserFunctions[tflite::BuiltinOperator_EXPAND_DIMS] =
      &TfLiteParserImpl::ParseExpandDims;
  m_ParserFunctions[tflite::BuiltinOperator_FULLY_CONNECTED] =
      &TfLiteParserImpl::ParseFullyConnected;
  m_ParserFunctions[tflite::BuiltinOperator_GATHER] =
      &TfLiteParserImpl::ParseGather;
  m_ParserFunctions[tflite::BuiltinOperator_GREATER] =
      &TfLiteParserImpl::ParseGreater;
  m_ParserFunctions[tflite::BuiltinOperator_GREATER_EQUAL] =
      &TfLiteParserImpl::ParseGreaterOrEqual;
  m_ParserFunctions[tflite::BuiltinOperator_HARD_SWISH] =
      &TfLiteParserImpl::ParseHardSwish;
  m_ParserFunctions[tflite::BuiltinOperator_LEAKY_RELU] =
      &TfLiteParserImpl::ParseLeakyRelu;
  m_ParserFunctions[tflite::BuiltinOperator_LESS] =
      &TfLiteParserImpl::ParseLess;
  m_ParserFunctions[tflite::BuiltinOperator_LESS_EQUAL] =
      &TfLiteParserImpl::ParseLessOrEqual;
  m_ParserFunctions[tflite::BuiltinOperator_LOCAL_RESPONSE_NORMALIZATION] =
      &TfLiteParserImpl::ParseLocalResponseNormalization;
  m_ParserFunctions[tflite::BuiltinOperator_LOGICAL_NOT] =
      &TfLiteParserImpl::ParseLogicalNot;
  m_ParserFunctions[tflite::BuiltinOperator_LOGISTIC] =
      &TfLiteParserImpl::ParseLogistic;
  m_ParserFunctions[tflite::BuiltinOperator_L2_NORMALIZATION] =
      &TfLiteParserImpl::ParseL2Normalization;
  m_ParserFunctions[tflite::BuiltinOperator_MAX_POOL_2D] =
      &TfLiteParserImpl::ParseMaxPool2D;
  m_ParserFunctions[tflite::BuiltinOperator_MAXIMUM] =
      &TfLiteParserImpl::ParseMaximum;
  m_ParserFunctions[tflite::BuiltinOperator_MEAN] =
      &TfLiteParserImpl::ParseMean;
  m_ParserFunctions[tflite::BuiltinOperator_MINIMUM] =
      &TfLiteParserImpl::ParseMinimum;
  m_ParserFunctions[tflite::BuiltinOperator_MIRROR_PAD] =
      &TfLiteParserImpl::ParseMirrorPad;
  ...
  m_ParserFunctions[tflite::BuiltinOperator_NOT_EQUAL] =
      &TfLiteParserImpl::ParseNotEqual;
  ...
  m_ParserFunctions[tflite::BuiltinOperator_QUANTIZE] =
      &TfLiteParserImpl::ParseQuantize;
  m_ParserFunctions[tflite::BuiltinOperator_RELU] =
      &TfLiteParserImpl::ParseRelu;
  m_ParserFunctions[tflite::BuiltinOperator_RELU6] =
      &TfLiteParserImpl::ParseRelu6;
  m_ParserFunctions[tflite::BuiltinOperator_REDUCE_MAX] =
      &TfLiteParserImpl::ParseReduceMax;
  m_ParserFunctions[tflite::BuiltinOperator_REDUCE_MIN] =
      &TfLiteParserImpl::ParseReduceMin;
  m_ParserFunctions[tflite::BuiltinOperator_REDUCE_PROD] =
      &TfLiteParserImpl::ParseReduceProd;
  m_ParserFunctions[tflite::BuiltinOperator_RESHAPE] =
      &TfLiteParserImpl::ParseReshape;
  m_ParserFunctions[tflite::BuiltinOperator_RESIZE_BILINEAR] =
      &TfLiteParserImpl::ParseResizeBilinear;
  m_ParserFunctions[tflite::BuiltinOperator_RESIZE_NEAREST_NEIGHBOR] =
  ...
  m_ParserFunctions[tflite::BuiltinOperator_SLICE] =
      &TfLiteParserImpl::ParseSlice;
  m_ParserFunctions[tflite::BuiltinOperator_SOFTMAX] =
      &TfLiteParserImpl::ParseSoftmax;
  m_ParserFunctions[tflite::BuiltinOperator_SPACE_TO_BATCH_ND] =
      &TfLiteParserImpl::ParseSpaceToBatchND;
  m_ParserFunctions[tflite::BuiltinOperator_SPLIT] =
      &TfLiteParserImpl::ParseSplit;
  m_ParserFunctions[tflite::BuiltinOperator_SPLIT_V] =
      &TfLiteParserImpl::ParseSplitV;
  m_ParserFunctions[tflite::BuiltinOperator_SQUEEZE] =
      &TfLiteParserImpl::ParseSqueeze;
  m_ParserFunctions[tflite::BuiltinOperator_STRIDED_SLICE] =
      &TfLiteParserImpl::ParseStridedSlice;
  ...
  m_ParserFunctions[tflite::BuiltinOperator_COS] = &TfLiteParserImpl::ParseCos;
  m_ParserFunctions[tflite::BuiltinOperator_SUM] = &TfLiteParserImpl::ParseSum;
  m_ParserFunctions[tflite::BuiltinOperator_TANH] =
      &TfLiteParserImpl::ParseTanH;
  m_ParserFunctions[tflite::BuiltinOperator_TRANSPOSE] =
      &TfLiteParserImpl::ParseTranspose;
  m_ParserFunctions[tflite::BuiltinOperator_TRANSPOSE_CONV] =
      &TfLiteParserImpl::ParseTransposeConv;
  m_ParserFunctions[tflite::BuiltinOperator_UNPACK] =
      &TfLiteParserImpl::ParseUnpack;

  // register supported custom operators
  m_CustomParserFunctions["TFLite_Detection_PostProcess"] =
      &TfLiteParserImpl::ParseDetectionPostProcess;
}
...
INetworkPtr
TfLiteParserImpl::CreateNetworkFromBinaryFile(const char *graphFile) {
  ResetParser();
  m_Model = LoadModelFromFile(graphFile);
  return CreateNetworkFromModel();
}

INetworkPtr TfLiteParserImpl::CreateNetworkFromBinary(
    const std::vector<uint8_t> &binaryContent) {
  ResetParser();
...
armnn::INetworkPtr
TfLiteParserImpl::LoadModel(std::unique_ptr<tflite::ModelT> model) {
  ResetParser();
...
INetworkPtr TfLiteParserImpl::CreateNetworkFromModel() {
  ...
  if (m_Model->subgraphs.size() != 1) {
    throw ParseException(fmt::format("Current TfLite parser only supports 1 "
                                     "subgraph. Current one has: {} {}",
...
// in a backwards compatible manner
  ...
  // establish the connections from the layer outputs to the inputs of the
  // subsequent layers
  for (subgraphIndex = 0; subgraphIndex < m_SubgraphConnections.size();
       ++subgraphIndex) {
    for (size_t tensorIndex = 0;
         tensorIndex < m_SubgraphConnections[subgraphIndex].size();
         ++tensorIndex) {
      if (m_SubgraphConnections[subgraphIndex][tensorIndex].outputSlot !=
          nullptr) {
        for (size_t inputSlotIdx = 0;
             inputSlotIdx < m_SubgraphConnections[subgraphIndex][tensorIndex]
                                .inputSlots.size();
             ++inputSlotIdx) {
          m_SubgraphConnections[subgraphIndex][tensorIndex].outputSlot->Connect(
              *(m_SubgraphConnections[subgraphIndex][tensorIndex]
                    .inputSlots[inputSlotIdx]));
        }
      }
    }
  }
...
void TfLiteParserImpl::RegisterProducerOfTensor(size_t subgraphIndex,
  ...
  if (tensorSlots.outputSlot != nullptr) {
    throw ParseException(fmt::format(
        ...
        "subgraph:{} tensor:{} {}",
        subgraphIndex, tensorIndex, CHECK_LOCATION().AsString()));
  }
...
void TfLiteParserImpl::ParseCustomOperator(size_t subgraphIndex,
  ...
  const auto &customCode =
      m_Model->operator_codes[operatorPtr->opcode_index]->custom_code;
...
void TfLiteParserImpl::ParseUnsupportedOperator(size_t subgraphIndex,
                                                size_t operatorIndex) {
  CHECK_MODEL(m_Model, subgraphIndex, operatorIndex);
...
// in a backwards compatible manner
#if defined(ARMNN_POST_TFLITE_2_3)
  auto opcode = std::max(
      m_Model->operator_codes[opcodeIndex]->builtin_code,
      static_cast<tflite::BuiltinOperator>(
  ...
  if (!m_Options || !m_Options.value().m_StandInLayerForUnsupported) {
    ...
    throw ParseException(fmt::format(
        ...
        "subgraph:{} operator:{} "
        "opcode_index:{} opcode:{} / {} {}",
        subgraphIndex, operatorIndex, opcodeIndex, opcode,
        tflite::EnumNameBuiltinOperator(opcode), CHECK_LOCATION().AsString()));
  }
  ...
  const unsigned int numInputs =
      armnn::numeric_cast<unsigned int>(inputs.size());
  const unsigned int numOutputs =
      armnn::numeric_cast<unsigned int>(outputs.size());

  StandInDescriptor descriptor(numInputs, numOutputs);
  auto layerName =
      fmt::format("StandIn:{}:{}:{}", subgraphIndex, operatorIndex, opcode);

  // Add a non-executable StandInLayer as a placeholder for any unsupported
  // operator
  IConnectableLayer *layer =
      m_Network->AddStandInLayer(descriptor, layerName.c_str());
  ...
  auto inputTensorIds = AsUnsignedVector(
      GetInputTensorIds(m_Model, subgraphIndex, operatorIndex));
  auto outputTensorIds = AsUnsignedVector(
...
void TfLiteParserImpl::ParseCast(size_t subgraphIndex, size_t operatorIndex) {
  ...
  auto inputTensorIndexes = AsUnsignedVector(
      GetInputTensorIds(m_Model, subgraphIndex, operatorIndex));
  RegisterInputSlots(subgraphIndex, operatorIndex, layer,
                     {inputTensorIndexes[0]});

  auto outputTensorIndexes = AsUnsignedVector(
      GetOutputTensorIds(m_Model, subgraphIndex, operatorIndex));
...
void TfLiteParserImpl::ParseConv2D(size_t subgraphIndex, size_t operatorIndex) {
  ...
  auto filterTensorAndData =
      CreateConstTensorNonPermuted(inputs[1], filterTensorInfo);
  ...
  if (inputs.size() == 3) {
    ...
    auto biasTensorAndData =
        CreateConstTensorNonPermuted(inputs[2], biasTensorInfo);
    layer = m_Network->AddConvolution2dLayer(
        desc, filterTensorAndData, Optional<ConstTensor>(biasTensorAndData),
        layerName.c_str());
  } else {
    layer = m_Network->AddConvolution2dLayer(
        desc, filterTensorAndData, EmptyOptional(), layerName.c_str());
  }
  ...
  // after all layers have been created only the tensors for the inputs are
  // relevant, exclude the const tensors
  auto inputTensorIndexes = AsUnsignedVector(
      GetInputTensorIds(m_Model, subgraphIndex, operatorIndex));
  RegisterInputSlots(subgraphIndex, operatorIndex, layer,
                     {inputTensorIndexes[0]});
...
void TfLiteParserImpl::ParseConv3D(size_t subgraphIndex, size_t operatorIndex) {
  ...
  // Assuming the filter is DHWIO : Depth, Height, Width, OutputChannels,
  // InputChannels
  unsigned int filterDepth = filterTensorInfo.GetShape()[0];
  unsigned int filterHeight = filterTensorInfo.GetShape()[1];
  unsigned int filterWidth = filterTensorInfo.GetShape()[2];

  CalcPadding(inputDepth, filterDepth, desc.m_StrideZ, desc.m_DilationY,
              desc.m_PadFront, desc.m_PadBack, options->padding);
  CalcPadding(inputHeight, filterHeight, desc.m_StrideY, desc.m_DilationY,
              desc.m_PadTop, desc.m_PadBottom, options->padding);
  CalcPadding(inputWidth, filterWidth, desc.m_StrideX, desc.m_DilationX,
              desc.m_PadLeft, desc.m_PadRight, options->padding);
  ...
  if (inputs.size() == 3) {
    ...
    // Add the biases input to the registration list, a constant layer will be
    // added by SetupConstantLayers.
    tensorIndexesToRegister.emplace_back(inputTensorIndexes[2]);
  }

  armnn::IConnectableLayer *layer =
      m_Network->AddConvolution3dLayer(desc, layerName.c_str());
  ARMNN_ASSERT(layer != nullptr);
...
void TfLiteParserImpl::ParseDepthwiseConv2D(size_t subgraphIndex,
                                            size_t operatorIndex) {
  CHECK_MODEL(m_Model, subgraphIndex, operatorIndex);
  ...
  // ArmNN uses the same filter tensor layout at TfLite [1, H, W, O] no need for
  // any permutation
  auto filterTensor = CreateConstTensorNonPermuted(inputs[1], filterTensorInfo);
  armnn::IConnectableLayer *layer = nullptr;
  auto layerName =
      fmt::format("DepthwiseConv2D:{}:{}", subgraphIndex, operatorIndex);
  ...
  if (inputs.size() == 3) {
    ...
    auto biasTensorAndData =
        CreateConstTensorNonPermuted(inputs[2], biasTensorInfo);
    layer = m_Network->AddDepthwiseConvolution2dLayer(
        desc, filterTensor, Optional<ConstTensor>(biasTensorAndData),
        layerName.c_str());
  } else {
    layer = m_Network->AddDepthwiseConvolution2dLayer(
        desc, filterTensor, EmptyOptional(), layerName.c_str());
  }
  ARMNN_ASSERT(layer != nullptr);
...
void TfLiteParserImpl::ParseDequantize(size_t subgraphIndex,
                                       size_t operatorIndex) {
  CHECK_MODEL(m_Model, subgraphIndex, operatorIndex);
  ...
  auto layerName =
      fmt::format("Dequantize:{}:{}", subgraphIndex, operatorIndex);
...
void TfLiteParserImpl::ParseExpandDims(size_t subgraphIndex,
                                       size_t operatorIndex) {
  CHECK_MODEL(m_Model, subgraphIndex, operatorIndex);
  ...
  auto layerName =
      fmt::format("ExpandDims:{}:{}", subgraphIndex, operatorIndex);

  armnn::TensorInfo inputTensorInfo = ToTensorInfo(inputs[0]);
  armnn::TensorInfo outputTensorInfo = ToTensorInfo(outputs[0], true);

  CheckMatchingQuantization(inputTensorInfo, outputTensorInfo, layerName,
                            "Input 0", "Output 0");
  ...
  if (outputTensorInfo.GetShape().AreAllDimensionsSpecified()) {
    ...
    int32_t inputDimSize =
        static_cast<int32_t>(inputTensorInfo.GetShape().GetNumDimensions());

    if (axis > inputDimSize || axis < 0 - (inputDimSize + 1)) {
      throw ParseException("axis must be in range [0 - (inputDimSize + 1), "
                           "inputDimSize] inclusive");
    }
    ...
    std::vector<unsigned int> shape(static_cast<unsigned int>(inputDimSize) +
                                    1);
    unsigned int inputShapeIndex = 0;
    for (unsigned int i = 0; i < static_cast<unsigned int>(inputDimSize + 1);
         ++i) {
    ...
    reshapeDesc.m_TargetShape =
        TensorShape(static_cast<unsigned int>(inputDimSize + 1), shape.data());
  }

  IConnectableLayer *layer =
      m_Network->AddReshapeLayer(reshapeDesc, layerName.c_str());
  ARMNN_ASSERT(layer != nullptr);
  layer->GetOutputSlot(0).SetTensorInfo(outputTensorInfo);
...
void TfLiteParserImpl::ParseTranspose(size_t subgraphIndex,
                                      size_t operatorIndex) {
  CHECK_MODEL(m_Model, subgraphIndex, operatorIndex);
  ...
  if (inputs.size() == 2) {
    ...
    ::memcpy(permuteShape.data(), permuteBufferPtr->data.data(),
             permuteTensorInfo.GetNumBytes());
    PermutationVector permutationVector(permuteShape.data(),
                                        permuteTensorInfo.GetNumElements());
  ...
  IConnectableLayer *layer =
      m_Network->AddTransposeLayer(desc, layerName.c_str());
  ARMNN_ASSERT(layer != nullptr);
  layer->GetOutputSlot(0).SetTensorInfo(outputTensorInfo);
...
void TfLiteParserImpl::ParseTransposeConv(size_t subgraphIndex,
                                          size_t operatorIndex) {
  CHECK_MODEL(m_Model, subgraphIndex, operatorIndex);
  ...
  if (inputs[0]) {
    ...
    if (tensorInfo.GetDataType() == DataType::Signed32) {
      ::memcpy(output_shape.data(),
               GetBuffer(m_Model, inputs[0]->buffer)->data.data(),
    ...
    // Change from signed to unsigned int to store in
  ...
  armnn::IConnectableLayer *layer = nullptr;
  auto layerName =
      fmt::format("TransposeConv:{}:{}", subgraphIndex, operatorIndex);

  if (desc.m_BiasEnabled) {
    auto biasTensorInfo = ToTensorInfo(inputs[3]);
    auto biasConstTensor =
        CreateConstTensorNonPermuted(inputs[3], biasTensorInfo);
    layer = m_Network->AddTransposeConvolution2dLayer(
        desc, filterTensorAndData, biasConstTensor, layerName.c_str());
  } else {
    layer = m_Network->AddTransposeConvolution2dLayer(
        desc, filterTensorAndData, EmptyOptional(), layerName.c_str());
  }
  ...
  // only the tensors for the inputs are relevant, exclude the const (filter)
  // tensor
  auto inputTensorIndexes = AsUnsignedVector(
      GetInputTensorIds(m_Model, subgraphIndex, operatorIndex));
  RegisterInputSlots(subgraphIndex, operatorIndex, layer,
...
void TfLiteParserImpl::ParseAveragePool2D(size_t subgraphIndex,
                                          size_t operatorIndex) {
  ParsePool(subgraphIndex, operatorIndex, PoolingAlgorithm::Average);
}

void TfLiteParserImpl::ParseBatchToSpaceND(size_t subgraphIndex,
                                           size_t operatorIndex) {
  CHECK_MODEL(m_Model, subgraphIndex, operatorIndex);
  ...
  ::memcpy(blockShape.data(), blockShapeBufferPtr->data.data(),
           blockShapeTensorInfo.GetNumBytes());

  std::vector<unsigned int> cropsVector(cropsTensorInfo.GetNumElements());
  ::memcpy(cropsVector.data(), cropsBufferPtr->data.data(),
           cropsTensorInfo.GetNumBytes());
  ...
  auto layerName =
      fmt::format("BatchToSpaceND:{}:{}", subgraphIndex, operatorIndex);
  ...
  IConnectableLayer *layer =
      m_Network->AddBatchToSpaceNdLayer(desc, layerName.c_str());
  ARMNN_ASSERT(layer != nullptr);
  layer->GetOutputSlot(0).SetTensorInfo(outputTensorInfo);
...
void TfLiteParserImpl::ParseL2Normalization(size_t subgraphIndex,
                                            size_t operatorIndex) {
  CHECK_MODEL(m_Model, subgraphIndex, operatorIndex);
  ...
  desc.m_DataLayout = armnn::DataLayout::NHWC;
  auto layerName =
      fmt::format("L2Normalization:{}:{}", subgraphIndex, operatorIndex);
  IConnectableLayer *layer =
      m_Network->AddL2NormalizationLayer(desc, layerName.c_str());
...
void TfLiteParserImpl::ParseMaxPool2D(size_t subgraphIndex,
                                      size_t operatorIndex) {
  ParsePool(subgraphIndex, operatorIndex, PoolingAlgorithm::Max);
}

void TfLiteParserImpl::ParseMaximum(size_t subgraphIndex,
                                    size_t operatorIndex) {
  CHECK_MODEL(m_Model, subgraphIndex, operatorIndex);
  ...
  CheckMatchingQuantization(inputTensorInfo, input1TensorInfo, layerName,
                            "Input 0", "Input 1");
  ...
  auto inputTensorIndexes = AsUnsignedVector(
      GetInputTensorIds(m_Model, subgraphIndex, operatorIndex));
  RegisterInputSlots(subgraphIndex, operatorIndex, layer,
                     {inputTensorIndexes[0], inputTensorIndexes[1]});
...
void TfLiteParserImpl::ParseMinimum(size_t subgraphIndex,
                                    size_t operatorIndex) {
  CHECK_MODEL(m_Model, subgraphIndex, operatorIndex);
...
void TfLiteParserImpl::ParsePool(size_t subgraphIndex, size_t operatorIndex,
  ...
  case PoolingAlgorithm::Max:
    layerName = fmt::format("MaxPool2D:{}:{}", subgraphIndex, operatorIndex);
    break;
  default:
  ...
  IConnectableLayer *layer =
      m_Network->AddPooling2dLayer(desc, layerName.c_str());
  ARMNN_ASSERT(layer != nullptr);
  layer->GetOutputSlot(0).SetTensorInfo(outputTensorInfo);
...
void TfLiteParserImpl::ParseSlice(size_t subgraphIndex, size_t operatorIndex) {
  ...
  ::memcpy(begin.data(), beginBufferPtr->data.data(),
           beginTensorInfo.GetNumBytes());
  ...
  ::memcpy(signedSize.data(), sizeBufferPtr->data.data(),
           sizeTensorInfo.GetNumBytes());
  ...
  for (unsigned int i = 0; i < signedSize.size(); ++i) {
    ...
    if (signedValue < -1 ||
        signedValue >
            static_cast<int>(inputTensorInfo.GetShape()[i] - begin[i])) {
      throw ParseException(
          fmt::format("Invalid value for size {} size must be in range "
                      "[-1, inputDimSize - begin] [-1, {}] inclusive {}",
                      signedValue, inputTensorInfo.GetShape()[i] - begin[i],
                      CHECK_LOCATION().AsString()));
    }
  ...
  IConnectableLayer *const layer =
      m_Network->AddSliceLayer(desc, layerName.c_str());
  layer->GetOutputSlot(0).SetTensorInfo(outputTensorInfo);
...
void TfLiteParserImpl::ParseSoftmax(size_t subgraphIndex,
                                    size_t operatorIndex) {
  CHECK_MODEL(m_Model, subgraphIndex, operatorIndex);
  const auto &operatorPtr =
      m_Model->subgraphs[subgraphIndex]->operators[operatorIndex];
  ...
  IConnectableLayer *const layer =
      m_Network->AddSoftmaxLayer(desc, layerName.c_str());
...
void TfLiteParserImpl::ParseSpaceToBatchND(size_t subgraphIndex,
                                           size_t operatorIndex) {
  CHECK_MODEL(m_Model, subgraphIndex, operatorIndex);
  ...
  ::memcpy(padListVector.data(), padListBufferPtr->data.data(),
  ...
  auto layerName =
      fmt::format("SpaceToBatchND:{}:{}", subgraphIndex, operatorIndex);
  ...
  IConnectableLayer *layer =
      m_Network->AddSpaceToBatchNdLayer(desc, layerName.c_str());
  ARMNN_ASSERT(layer != nullptr);
  layer->GetOutputSlot(0).SetTensorInfo(outputTensorInfo);
...
armnn::TensorInfo TfLiteParserImpl::OutputShapeOfSqueeze(
  ...
  TensorShape outShape = TensorShape(
...
void TfLiteParserImpl::ParseShape(size_t subgraphIndex, size_t operatorIndex) {
  ...
  if (outputTensorInfo.GetDataType() != armnn::DataType::Signed32 &&
    ...
    throw ParseException(
        fmt::format("Output tensor data type is not supported. (Supported "
                    "types: Signed32 & Signed64) {}",
                    CHECK_LOCATION().AsString()));
  }
...
void TfLiteParserImpl::ParseSqueeze(size_t subgraphIndex,
                                    size_t operatorIndex) {
  CHECK_MODEL(m_Model, subgraphIndex, operatorIndex);
  ...
  armnn::TensorInfo outputTensorInfo =
      TfLiteParserImpl::OutputShapeOfSqueeze(squeezeDim, inputTensorInfo);
...
void TfLiteParserImpl::ParseStridedSlice(size_t subgraphIndex,
                                         size_t operatorIndex) {
  CHECK_MODEL(m_Model, subgraphIndex, operatorIndex);
  ...
  ::memcpy(stride.data(), strideBufferPtr->data.data(),
           strideTensorInfo.GetNumBytes());
  ...
  auto layerName =
      fmt::format("StridedSlice:{}:{}", subgraphIndex, operatorIndex);
  IConnectableLayer *layer =
      m_Network->AddStridedSliceLayer(desc, layerName.c_str());
  ARMNN_ASSERT(layer != nullptr);
...
void TfLiteParserImpl::ParseCos(size_t subgraphIndex, size_t operatorIndex) {
  ParseElementwiseUnary(subgraphIndex, operatorIndex,
                        armnn::UnaryOperation::Cos);
}
...
void TfLiteParserImpl::ParseMul(size_t subgraphIndex, size_t operatorIndex) {
  ...
  IConnectableLayer *layer =
      m_Network->AddMultiplicationLayer(layerName.c_str());
  ARMNN_ASSERT(layer != nullptr);
...
void TfLiteParserImpl::ParsePad(size_t subgraphIndex, size_t operatorIndex) {
  ...
  TfLiteParserImpl::TensorRawPtrVector inputs =
      GetInputs(m_Model, subgraphIndex, operatorIndex);

  TfLiteParserImpl::TensorRawPtrVector outputs =
...
void TfLiteParserImpl::ParseMirrorPad(size_t subgraphIndex,
                                      size_t operatorIndex) {
  CHECK_MODEL(m_Model, subgraphIndex, operatorIndex);
  ...
  if (options->mode == tflite::MirrorPadMode_REFLECT) {
    ...
    ARMNN_THROW_PARSE_EXCEPTION(
        "PaddingMode must be either REFLECT or SYMMETRIC");
  }

  // If padding mode is Reflect then both paddings must be no greater than
  // inputShape(i) - 1. If padding mode is Symmetric then both paddings must be
  // no greater than inputShape(i).
  auto inputShape = inputTensorInfo.GetShape();
  auto padList = desc.m_PadList;

  const unsigned int isReflect =
      static_cast<unsigned int>(desc.m_PaddingMode == PaddingMode::Reflect);
...
void TfLiteParserImpl::ParsePrelu(size_t subgraphIndex, size_t operatorIndex) {
  ...
  if (IsConstTensor(inputs[1])) {
    ...
    auto alphaTensorAndData =
        CreateConstTensorNonPermuted(inputs[1], alphaTensorInfo);
    ...
    RegisterOutputSlots(subgraphIndex, VIRTUAL_OPERATOR_ID, constLayer,
                        {inputTensorIndexes[1]});
  } else {
    auto inputTensorIndexes = AsUnsignedVector(
        GetInputTensorIds(m_Model, subgraphIndex, operatorIndex));
...
void TfLiteParserImpl::ParseQuantize(size_t subgraphIndex,
                                     size_t operatorIndex) {
  CHECK_MODEL(m_Model, subgraphIndex, operatorIndex);
...
void TfLiteParserImpl::ParseRelu6(size_t subgraphIndex, size_t operatorIndex) {
  ParseActivation(subgraphIndex, operatorIndex,
                  ActivationFunction::BoundedReLu);
}

void TfLiteParserImpl::ParseLeakyRelu(size_t subgraphIndex,
                                      size_t operatorIndex) {
  ParseActivation(subgraphIndex, operatorIndex, ActivationFunction::LeakyReLu);
}

void TfLiteParserImpl::ParseLogistic(size_t subgraphIndex,
                                     size_t operatorIndex) {
...
void TfLiteParserImpl::ParseHardSwish(size_t subgraphIndex,
                                      size_t operatorIndex) {
  ParseActivation(subgraphIndex, operatorIndex, ActivationFunction::HardSwish);
}

void TfLiteParserImpl::ParseActivation(size_t subgraphIndex,
                                       size_t operatorIndex,
...
armnn::TensorInfo TfLiteParserImpl::OutputShapeOfReshape(
    const armnn::TensorInfo &inputTensorInfo,
    const std::vector<int32_t> &targetDimsIn) {
  std::vector<unsigned int> outputDims(targetDimsIn.begin(),
                                       targetDimsIn.end());
  const auto stretchDim =
      std::find(targetDimsIn.begin(), targetDimsIn.end(), -1);

  if (stretchDim != targetDimsIn.end()) {
    if (std::find(std::next(stretchDim), targetDimsIn.end(), -1) !=
        targetDimsIn.end()) {
      throw ParseException(
    ...
    auto targetNumElements = armnn::numeric_cast<unsigned int>(
        std::accumulate(targetDimsIn.begin(), targetDimsIn.end(), -1,
                        std::multiplies<int32_t>()));

    auto stretchIndex =
        static_cast<size_t>(std::distance(targetDimsIn.begin(), stretchDim));
    outputDims[stretchIndex] =
        inputTensorInfo.GetNumElements() / targetNumElements;
  }
...
void TfLiteParserImpl::ParseReshape(size_t subgraphIndex,
                                    size_t operatorIndex) {
  CHECK_MODEL(m_Model, subgraphIndex, operatorIndex);
  ...
  CheckMatchingQuantization(inputTensorInfo, actualOutputTensorInfo, layerName,
                            "Input 0", "Output 0");
  ...
  // If there is no built-in option given or if the built-in new_shape parameter
  // was empty
  if (!targetShapeFound) {
    // Check for a second input tensor
    if (inputs.size() > 1 && inputs[1] != nullptr) {
      if (inputs[1]->is_variable) {
        ARMNN_THROW_PARSE_EXCEPTION("Target shapes defined in non-const input "
      ...
      if (inputs[1]->type != tflite::TensorType_INT32) {
        ARMNN_THROW_PARSE_EXCEPTION(
            "Target 'shape' input is not an int32 type");
      }
      ...
      if (!values) {
        ARMNN_THROW_PARSE_EXCEPTION(
            "Reshape operator target shape input buffer data is null");
      }
  ...
  const armnn::TensorShape &reshapeOutputTensorShape =
      reshapeOutputTensorInfo.GetShape();
  if (inputs.size() > 1 &&
      !CheckShape(reshapeOutputTensorShape, outputs[0]->shape)) {
    std::stringstream ss;
    ss << "New shape defined in reshape parameters " << reshapeOutputTensorShape
       << " does not equal output shape " << actualOutputTensorInfo.GetShape()
       << ": " << CHECK_LOCATION().AsString();
    throw ParseException(ss.str());
  }
...
void TfLiteParserImpl::ParseResizeBilinear(size_t subgraphIndex,
                                           size_t operatorIndex) {
  ParseResize(subgraphIndex, operatorIndex, ResizeMethod::Bilinear);
}

void TfLiteParserImpl::ParseResizeNearestNeighbor(size_t subgraphIndex,
                                                  size_t operatorIndex) {
  ParseResize(subgraphIndex, operatorIndex, ResizeMethod::NearestNeighbor);
}

void TfLiteParserImpl::ParseResize(size_t subgraphIndex, size_t operatorIndex,
  ...
  ::memcpy(sizeTensorData.data(), sizeBufferPtr->data.data(),
           sizeTensorInfo.GetNumBytes());
  ...
  case ResizeMethod::Bilinear: {
    ...
    const auto &operatorPtr =
        m_Model->subgraphs[subgraphIndex]->operators[operatorIndex];
    const auto *options =
        operatorPtr->builtin_options.AsResizeBilinearOptions();
  ...
  case ResizeMethod::NearestNeighbor: {
    layerName +=
        fmt::format("NEARESTNEIGHBOR:{}:{}", subgraphIndex, operatorIndex);
    break;
  }
  default: {
    throw ParseException(fmt::format(
...
void TfLiteParserImpl::ParseConcatenation(size_t subgraphIndex,
                                          size_t operatorIndex) {
  CHECK_MODEL(m_Model, subgraphIndex, operatorIndex);
  ...
  const unsigned int concatDimInput =
      static_cast<unsigned int>((static_cast<int>(inputRank) + options->axis) %
                                static_cast<int>(inputRank));

  OriginsDescriptor concatDescriptor(static_cast<uint32_t>(numConcatView),
                                     inputRank);
  ...
  for (unsigned int viewIndex = 0; viewIndex < numConcatView; ++viewIndex) {
    ...
    armnnUtils::ProcessConcatInputTensorInfo(inputTensorInfo, concatDescriptor,
                                             concatDimInput, viewIndex,
                                             mergeDimOrigin);
  }

  auto layerName =
      fmt::format("Concatenation:{}:{}", subgraphIndex, operatorIndex);
  TensorInfo outputTensorInfo = ToTensorInfo(outputs[0], true);

  IConnectableLayer *layer =
      m_Network->AddConcatLayer(concatDescriptor, layerName.c_str());
  ARMNN_ASSERT(layer != nullptr);
  layer->GetOutputSlot(0).SetTensorInfo(outputTensorInfo);
...
void TfLiteParserImpl::ParseFullyConnected(size_t subgraphIndex,
                                           size_t operatorIndex) {
  CHECK_MODEL(m_Model, subgraphIndex, operatorIndex);

  const auto &operatorRfr =
      m_Model->subgraphs[subgraphIndex]->operators[operatorIndex];
  ...
  int32_t weightsDimension =
      static_cast<int32_t>(filterTensorInfo.GetNumDimensions());
  if (weightsDimension != 2) {
    throw ParseException(fmt::format(
        "Dimension {} for Fully Connected weights is not supported by Armnn. "
        "Node {}",
        weightsDimension, CHECK_LOCATION().AsString()));
  }

  armnn::IConnectableLayer *layer = nullptr;
  auto layerName =
      fmt::format("FullyConnected:{}:{}", subgraphIndex, operatorIndex);
  ...
  // Add the weights input to the registration list, constant layers will be
  // added by SetupConstantLayers if constant.
  tensorIndexesToRegister.emplace_back(inputTensorIndexes[1]);

  if (inputs.size() == 3) {
    desc.m_BiasEnabled = true;

    // Add the biases input to the registration list, constant layer will be
    // added by SetupConstantLayers.
    tensorIndexesToRegister.emplace_back(inputTensorIndexes[2]);
  }
  ...
  if (inputTensorInfo.GetNumDimensions() > 2) {
    ...
    // and "batch_size" is calculated by dividing the number of elements by
    // "input_size".
    std::vector<unsigned int> reshapedDimensions(2);
    reshapedDimensions[1] = filterTensorInfo.GetShape()[1];
    reshapedDimensions[0] =
        inputTensorInfo.GetNumElements() / reshapedDimensions[1];

    if (inputTensorInfo.GetNumElements() % reshapedDimensions[1] != 0) {
      throw ParseException(fmt::format(
          "Failed to deduce input tensor shape from filter size {} {}",
          reshapedDimensions[1], CHECK_LOCATION().AsString()));
    }

    armnn::TensorInfo reshapedTensorInfo = ToTensorInfo(inputs[0]);
    reshapedTensorInfo.SetShape(
        armnn::TensorShape{2, reshapedDimensions.data()});

    std::string reshapeLayerName =
        fmt::format("Reshape_for:{}", layer->GetName());
    armnn::ReshapeDescriptor reshapeDescriptor;
    reshapeDescriptor.m_TargetShape = reshapedTensorInfo.GetShape();
    armnn::IConnectableLayer *reshapeLayer =
        m_Network->AddReshapeLayer(reshapeDescriptor, layerName.c_str());

    reshapeLayer->GetOutputSlot(0).SetTensorInfo(reshapedTensorInfo);
    reshapeLayer->GetOutputSlot(0).Connect(layer->GetInputSlot(0));

    RegisterInputSlots(subgraphIndex, operatorIndex, reshapeLayer,
                       {inputTensorIndexes[0]});
    // Fc layer connects to the reshape layer, so we skip the first input slot
    // when registering fc's input slots
    tensorIndexesToRegister.erase(tensorIndexesToRegister.begin());
    startingSlotIndex = 1;
  }

  RegisterInputSlots(subgraphIndex, operatorIndex, layer,
                     tensorIndexesToRegister, startingSlotIndex);

  armnn::TensorInfo outputTensorInfo = ToTensorInfo(outputs[0], true);
  layer->GetOutputSlot(0).SetTensorInfo(outputTensorInfo);

  // we need to add the activation layer and fortunately we don't need to care
  // about the data layout
  armnn::IConnectableLayer *fusedActivationLayer =
      AddFusedActivationLayer(layer, 0, options->fused_activation_function);

  // register the output connection slots for the layer, connections are made
  // after all layers have been created
  auto outputTensorIndexes = AsUnsignedVector(
      GetOutputTensorIds(m_Model, subgraphIndex, operatorIndex));
  RegisterOutputSlots(subgraphIndex, operatorIndex, fusedActivationLayer,
                      {outputTensorIndexes[0]});
}

void TfLiteParserImpl::ParseDetectionPostProcess(size_t subgraphIndex,
                                                 size_t operatorIndex) {
  CHECK_MODEL(m_Model, subgraphIndex, operatorIndex);
  ...
  auto anchorTensorAndData =
      CreateConstTensorNonPermuted(inputs[2], anchorTensorInfo);

  auto layerName =
      fmt::format("DetectionPostProcess:{}:{}", subgraphIndex, operatorIndex);
  IConnectableLayer *layer = m_Network->AddDetectionPostProcessLayer(
      desc, anchorTensorAndData, layerName.c_str());

  ARMNN_ASSERT(layer != nullptr);

  // The model does not specify the output shapes.
  // The output shapes are calculated from the max_detection and
  // max_classes_per_detection.
  unsigned int numDetectedBox =
      desc.m_MaxDetections * desc.m_MaxClassesPerDetection;
  ...
  for (unsigned int i = 0; i < outputs.size(); ++i) {
    armnn::TensorInfo detectionBoxOutputTensorInfo =
        ToTensorInfo(outputs[i], m_OverridenOutputShapes[i]);
  ...
  auto outputTensorIndexes = AsUnsignedVector(
      GetOutputTensorIds(m_Model, subgraphIndex, operatorIndex));
  RegisterOutputSlots(subgraphIndex, operatorIndex, layer,
                      {outputTensorIndexes[0], outputTensorIndexes[1],
                       outputTensorIndexes[2], outputTensorIndexes[3]});
}
...
void TfLiteParserImpl::ParsePack(size_t subgraphIndex, size_t operatorIndex) {
  ...
  // Use the tensor shape of the first input as the "correct" input shape in the
  // descriptor
  armnn::TensorInfo inputTensorInfo = ToTensorInfo(inputs[0]);
...
void TfLiteParserImpl::ParseUnpack(size_t subgraphIndex, size_t operatorIndex) {
  ...
  if (unpackAxis >= inputTensorInfo.GetNumDimensions()) {
    throw ParseException(
        fmt::format("The unpack axis: {} cannot be greater than or equal to "
                    ...
                    unpackAxis, inputTensorInfo.GetNumDimensions(),
                    CHECK_LOCATION().AsString()));
  }

  unsigned int unpackNum = CHECKED_NON_NEGATIVE(options->num);
  // If num is not defined, automatically infer from the length of the dimension
  // axis.
  if (unpackNum == 0) {
    unpackNum = inputTensorInfo.GetShape()[unpackAxis];
  }

  // If unpack number cannot be inferred and is still zero, throw
  // ParseException.
  if (unpackNum == 0) {
  ...
  SplitterDescriptor splitDesc(
      unpackNum, static_cast<unsigned int>(unpackDimSizes.size()));
  ...
  IConnectableLayer *layer =
      m_Network->AddSplitterLayer(splitDesc, layerName.c_str());
  ARMNN_ASSERT(layer != nullptr);

  TensorShape splitOutShape = TensorShape(
      static_cast<unsigned int>(unpackDimSizes.size()), unpackDimSizes.data());
  ...
  TensorShape reshapeOutputShape(splitOutShape.GetNumDimensions() - 1,
                                 reshapeDims.data());

  // Create reshape to remove the unpacked dimension for unpack operator of each
  ...
  for (unsigned int k = 0; k < layer->GetNumOutputSlots(); ++k) {
    ...
    armnn::IConnectableLayer *reshapeLayer =
        m_Network->AddReshapeLayer(desc, layerName.c_str());

    layer->GetOutputSlot(k).SetTensorInfo(
        armnn::TensorInfo(splitOutShape, outputTensorInfo.GetDataType(),
...
void TfLiteParserImpl::ParseSplit(size_t subgraphIndex, size_t operatorIndex) {
  ...
  ::memcpy(axisData.data(), axisBufferPtr->data.data(),
           axisTensorInfo.GetNumBytes());
  int32_t axis = axisData[0];

  auto inputDimensions =
      static_cast<int32_t>(inputTensorInfo.GetNumDimensions());
  if (((axis < -inputDimensions) && (axis < 0)) ||
      ((axis >= inputDimensions) && (axis > 0))) {
    ...
    throw ParseException(fmt::format(
        ...
        axis, CHECK_LOCATION().AsString()));
  }

  const unsigned int splitDim =
      armnnUtils::GetUnsignedAxis(inputTensorInfo.GetNumDimensions(), axis);

  auto inputDimSize = inputTensorInfo.GetNumDimensions();
  if (inputDimSize > MaxNumOfTensorDimensions) {
    throw ParseException(
        fmt::format("The number of dimensions: {} for input tensors of the "
                    "split op cannot be greater than {} {}",
                    inputTensorInfo.GetNumDimensions(),
                    MaxNumOfTensorDimensions, CHECK_LOCATION().AsString()));
  }
...
void TfLiteParserImpl::ParseSplitV(size_t subgraphIndex, size_t operatorIndex) {
  ...
  const unsigned int splitDim =
      ComputeWrappedIndex(axis, inputTensorInfo.GetNumDimensions());
  ...
  ::memcpy(splitsData.data(), splitsBufferPtr->data.data(),
  ...
  if (numInferred == 0) {
    if (splitSum !=
        armnn::numeric_cast<int>(inputTensorInfo.GetShape()[splitDim])) {
      throw ParseException("SplitV split_sizes does not sum to the dimension "
                           "of value along split_dim.");
    }
  ...
  for (unsigned int j = 0; j < numSplits; ++j) {
    ...
    // Set the size of the views.
    for (unsigned int dimIdx = 0; dimIdx < inputTensorInfo.GetNumDimensions();
         ++dimIdx) {
...
void TfLiteParserImpl::ParseArgMinMax(size_t subgraphIndex,
                                      size_t operatorIndex,
  ...
  auto layerName = argMinMaxFunction == ArgMinMaxFunction::Max ? "ArgMax:{}:{}"
                                                               : "ArgMin:{}:{}";
  auto layerNameFormatted =
      fmt::format(layerName, subgraphIndex, operatorIndex);
  IConnectableLayer *layer =
      m_Network->AddArgMinMaxLayer(desc, layerNameFormatted.c_str());
  ARMNN_ASSERT(layer != nullptr);
  layer->GetOutputSlot(0).SetTensorInfo(outputTensorInfo);
...
void TfLiteParserImpl::ParseGather(size_t subgraphIndex, size_t operatorIndex) {
  ...
  IConnectableLayer *layer =
      m_Network->AddGatherLayer(gatherDescriptor, layerName.c_str());
  ARMNN_ASSERT(layer != nullptr);
  layer->GetOutputSlot(0).SetTensorInfo(outputTensorInfo);
...
void TfLiteParserImpl::ParseDepthToSpace(size_t subgraphIndex,
                                         size_t operatorIndex) {
  CHECK_MODEL(m_Model, subgraphIndex, operatorIndex);
  ...
  if (blockSize < 2) {
    throw ParseException(fmt::format(
        ...
        blockSize, CHECK_LOCATION().AsString()));
  }
  descriptor.m_BlockSize = armnn::numeric_cast<uint32_t>(blockSize);

  auto layerName =
      fmt::format("DepthToSpace:{}:{}", subgraphIndex, operatorIndex);
  IConnectableLayer *layer =
      m_Network->AddDepthToSpaceLayer(descriptor, layerName.c_str());
  ARMNN_ASSERT(layer != nullptr);
  TensorInfo outputTensorInfo = ToTensorInfo(outputs[0], true);
  layer->GetOutputSlot(0).SetTensorInfo(outputTensorInfo);
...
void TfLiteParserImpl::ParseReduceProd(size_t subgraphIndex,
                                       size_t operatorIndex) {
  ParseReduce(subgraphIndex, operatorIndex, armnn::ReduceOperation::Prod);
}

void TfLiteParserImpl::ParseReduceMax(size_t subgraphIndex,
                                      size_t operatorIndex) {
  ParseReduce(subgraphIndex, operatorIndex, armnn::ReduceOperation::Max);
}

void TfLiteParserImpl::ParseReduceMin(size_t subgraphIndex,
                                      size_t operatorIndex) {
  ParseReduce(subgraphIndex, operatorIndex, armnn::ReduceOperation::Min);
}

void TfLiteParserImpl::ParseReduce(size_t subgraphIndex, size_t operatorIndex,
...
void TfLiteParserImpl::ParseLocalResponseNormalization(size_t subgraphIndex,
                                                       size_t operatorIndex) {
  CHECK_MODEL(m_Model, subgraphIndex, operatorIndex);
  ...
  const auto &operatorPtr =
      m_Model->subgraphs[subgraphIndex]->operators[operatorIndex];
  const auto *options =
      operatorPtr->builtin_options.AsLocalResponseNormalizationOptions();

  armnn::NormalizationDescriptor descriptor;
  descriptor.m_DataLayout = armnn::DataLayout::NHWC;
  descriptor.m_NormChannelType = armnn::NormalizationAlgorithmChannel::Across;
  descriptor.m_NormMethodType =
  ...
  IConnectableLayer *layer =
      m_Network->AddNormalizationLayer(descriptor, layerNameFormatted.c_str());
  ARMNN_ASSERT(layer != nullptr);
...
void TfLiteParserImpl::ParseLogicalNot(size_t subgraphIndex,
                                       size_t operatorIndex) {
  ParseElementwiseUnary(subgraphIndex, operatorIndex,
...
void TfLiteParserImpl::ParseElementwiseUnary(size_t subgraphIndex,
  ...
  std::string layerName =
      std::string(GetUnaryOperationAsCString(unaryOperation)) + ":{}:{}";
  std::string layerNameFormatted =
      fmt::format(layerName, subgraphIndex, operatorIndex);
  ...
  IConnectableLayer *layer =
      m_Network->AddElementwiseUnaryLayer(desc, layerNameFormatted.c_str());
  ARMNN_ASSERT(layer != nullptr);
...
void TfLiteParserImpl::ParseEqual(size_t subgraphIndex, size_t operatorIndex) {
  ParseComparison(subgraphIndex, operatorIndex,
                  armnn::ComparisonOperation::Equal);
}

void TfLiteParserImpl::ParseNotEqual(size_t subgraphIndex,
                                     size_t operatorIndex) {
  ParseComparison(subgraphIndex, operatorIndex,
                  armnn::ComparisonOperation::NotEqual);
}

void TfLiteParserImpl::ParseGreater(size_t subgraphIndex,
                                    size_t operatorIndex) {
  ParseComparison(subgraphIndex, operatorIndex,
                  armnn::ComparisonOperation::Greater);
}

void TfLiteParserImpl::ParseGreaterOrEqual(size_t subgraphIndex,
                                           size_t operatorIndex) {
  ParseComparison(subgraphIndex, operatorIndex,
...
void TfLiteParserImpl::ParseLessOrEqual(size_t subgraphIndex,
                                        size_t operatorIndex) {
  ParseComparison(subgraphIndex, operatorIndex,
                  armnn::ComparisonOperation::LessOrEqual);
}

void TfLiteParserImpl::ParseComparison(
  ...
  auto layerName =
      std::string(GetComparisonOperationAsCString(comparisonOperation)) +
      ":{}:{}";
  std::string layerNameFormatted =
      fmt::format(layerName, subgraphIndex, operatorIndex);
  ...
  armnn::TensorInfo inputTensorInfo = ToTensorInfo(inputs[0]);
  armnn::TensorInfo input1TensorInfo = ToTensorInfo(inputs[1]);
  CheckMatchingQuantization(inputTensorInfo, input1TensorInfo,
                            layerNameFormatted, "Input 0", "Input 1");

  ComparisonDescriptor desc;
  desc.m_Operation = comparisonOperation;
  IConnectableLayer *layer =
      m_Network->AddComparisonLayer(desc, layerNameFormatted.c_str());
  ARMNN_ASSERT(layer != nullptr);
...
armnn::IConnectableLayer *TfLiteParserImpl::AddFusedActivationLayer(
  ...
  default: {
    throw ParseException(fmt::format(
        "TfLite parser doesn't suppport fused activation: "
        "{}/{} {} ",
        activationType, tflite::EnumNameActivationFunctionType(activationType),
        CHECK_LOCATION().AsString()));
  }
  }
...
TfLiteParserImpl::ModelPtr
TfLiteParserImpl::LoadModelFromFile(const char *fileName) {
  if (fileName == nullptr) {
  ...
  return LoadModelFromBinary(
      reinterpret_cast<const uint8_t *>(fileContent.c_str()),
      fileContent.size());
}

TfLiteParserImpl::ModelPtr
TfLiteParserImpl::LoadModelFromBinary(const uint8_t *binaryContent,
                                      size_t len) {
  if (binaryContent == nullptr) {
    throw InvalidArgumentException(fmt::format(
        "Invalid (null) binary content {}", CHECK_LOCATION().AsString()));
  }
  ...
  if (verifier.VerifyBuffer<tflite::Model>() == false) {
    throw ParseException(
        fmt::format("Buffer doesn't conform to the expected Tensorflow Lite "
                    ...
                    len, CHECK_LOCATION().AsString()));
  }
  return tflite::UnPackModel(binaryContent);
}

TfLiteParserImpl::TensorRawPtrVector
TfLiteParserImpl::GetInputs(const ModelPtr &model, size_t subgraphIndex,
                            size_t operatorIndex) {
...
TfLiteParserImpl::TensorRawPtrVector
TfLiteParserImpl::GetOutputs(const ModelPtr &model, size_t subgraphIndex,
                             size_t operatorIndex) {
  CHECK_MODEL(model, subgraphIndex, operatorIndex);
...
TfLiteParserImpl::TensorIdRawPtrVector
TfLiteParserImpl::GetSubgraphInputs(const ModelPtr &model,
...
TfLiteParserImpl::TensorIdRawPtrVector
TfLiteParserImpl::GetSubgraphOutputs(const ModelPtr &model,
                                     size_t subgraphIndex) {
  CHECK_SUBGRAPH(model, subgraphIndex);
  const auto &subgraphPtr = model->subgraphs[subgraphIndex];
...
std::vector<int32_t> &
TfLiteParserImpl::GetInputTensorIds(const ModelPtr &model, size_t subgraphIndex,
                                    size_t operatorIndex) {
  CHECK_MODEL(model, subgraphIndex, operatorIndex);
  const auto &subgraphPtr = model->subgraphs[subgraphIndex];
  const auto &operatorPtr = subgraphPtr->operators[operatorIndex];
...
std::vector<int32_t> &TfLiteParserImpl::GetOutputTensorIds(
    const ModelPtr &model, size_t subgraphIndex, size_t operatorIndex) {
  CHECK_MODEL(model, subgraphIndex, operatorIndex);
  const auto &subgraphPtr = model->subgraphs[subgraphIndex];
  const auto &operatorPtr = subgraphPtr->operators[operatorIndex];
...
void TfLiteParserImpl::RegisterInputSlots(
    size_t subgraphIndex, size_t operatorIndex, IConnectableLayer *layer,
  ...
  if (tensorIndexes.size() + startingSlotIndex != layer->GetNumInputSlots()) {
    throw ParseException(
        fmt::format("The number of tensor inputs ({}) does not match the "
                    "number expected ({})"
                    " for subgraph:{} operator index:{} {}",
                    tensorIndexes.size(), layer->GetNumInputSlots(),
                    subgraphIndex, operatorIndex, CHECK_LOCATION().AsString()));
  }
...
void TfLiteParserImpl::RegisterOutputSlots(
    size_t subgraphIndex, size_t operatorIndex, IConnectableLayer *layer,
    const std::vector<unsigned int> &tensorIndexes) {
  CHECK_MODEL(m_Model, subgraphIndex, operatorIndex);
  ARMNN_ASSERT(layer != nullptr);
  if (tensorIndexes.size() != layer->GetNumOutputSlots()) {
    throw ParseException(
        fmt::format("The number of tensor outputs ({}) does not match the "
                    "number expected ({})"
                    " for subgraph:{} operator index:{} {}",
...
void TfLiteParserImpl::SetupInputLayers(size_t subgraphIndex) {
  ...
  for (auto const &tensorIdAndPtr : inputs) {
    auto bindingId =
        GenerateLayerBindingId(subgraphIndex, tensorIdAndPtr.first);
    IConnectableLayer *layer = m_Network->AddInputLayer(
        bindingId, tensorIdAndPtr.second->name.c_str());

    auto tensorInfo = ToTensorInfo(tensorIdAndPtr.second);
    layer->GetOutputSlot(0).SetTensorInfo(tensorInfo);

    RegisterOutputSlots(subgraphIndex, VIRTUAL_OPERATOR_ID, layer,
...
void TfLiteParserImpl::SetupOutputLayers(size_t subgraphIndex) {
  ...
  for (auto const &tensorIdAndPtr : outputs) {
    auto bindingId =
        GenerateLayerBindingId(subgraphIndex, tensorIdAndPtr.first);
    IConnectableLayer *layer = m_Network->AddOutputLayer(
        bindingId, tensorIdAndPtr.second->name.c_str());

    RegisterInputSlots(subgraphIndex, VIRTUAL_OPERATOR_ID, layer,
                       {static_cast<uint32_t>(tensorIdAndPtr.first)});
  }
}
...
void TfLiteParserImpl::SetupConstantLayers(size_t subgraphIndex) {
  ...
  for (unsigned int subgraphIndex = 0;
       subgraphIndex < m_SubgraphConnections.size(); ++subgraphIndex) {
    for (unsigned int tensorIndex = 0;
         tensorIndex < m_SubgraphConnections[subgraphIndex].size();
         ++tensorIndex) {
      if (m_SubgraphConnections[subgraphIndex][tensorIndex].outputSlot ==
              nullptr &&
          m_SubgraphConnections[subgraphIndex][tensorIndex].inputSlots.size() >
              0) {
        ...
        if (IsConstTensor(tensorPtr)) {
          ...
          auto tensorAndData =
              CreateConstTensorNonPermuted(tensorPtr, tensorInfo);
...
// example usage: BufferRawPtr bufferPtr = GetBuffer(m_Model,
// inputs[0]->buffer);
TfLiteParserImpl::BufferRawPtr
TfLiteParserImpl::GetBuffer(const ModelPtr &model, size_t bufferIndex) {
...
TfLiteParserImpl::CreateConstTensorAndStoreData(
    TfLiteParserImpl::BufferRawPtr bufferPtr,
    TfLiteParserImpl::TensorRawPtr tensorPtr, armnn::TensorInfo &tensorInfo,
    armnn::Optional<armnn::PermutationVector &> permutationVector) {
  // Make sure isConstant flag is set.
  ...
  auto constData = CreateConstTensorImpl<T>(bufferPtr, tensorPtr, tensorInfo,
                                            permutationVector);
...
std::pair<armnn::ConstTensor, TfLiteParserImpl::SupportedDataStorage>
TfLiteParserImpl::CreateConstTensorPermuted(
  ...
  case armnn::DataType::Float32:
    return CreateConstTensorAndStoreData<float>(bufferPtr, tensorPtr,
                                                tensorInfo, permutationVector);
  case armnn::DataType::QAsymmU8:
    return CreateConstTensorAndStoreData<uint8_t>(
        bufferPtr, tensorPtr, tensorInfo, permutationVector);
  case armnn::DataType::QSymmS8:
    return CreateConstTensorAndStoreData<int8_t>(bufferPtr, tensorPtr,
                                                 tensorInfo, permutationVector);
  case armnn::DataType::QAsymmS8:
    return CreateConstTensorAndStoreData<int8_t>(bufferPtr, tensorPtr,
                                                 tensorInfo, permutationVector);
  case armnn::DataType::Signed32:
    return CreateConstTensorAndStoreData<int32_t>(
        bufferPtr, tensorPtr, tensorInfo, permutationVector);
  default: {
...
BindingPointInfo
TfLiteParserImpl::GetNetworkInputBindingInfo(size_t subgraphId,
                                             const std::string &name) const {
  ...
  for (auto const &input : inputs) {
    if (input.second->name == name) {
      ...
      // Input tensors are always treated as constant tensors during network
      // execution.
  ...
  throw ParseException(fmt::format(
      ...
      "Possible inputs are: [{}] {}",
      subgraphId, name, bindings.str(), CHECK_LOCATION().AsString()));
}

BindingPointInfo
TfLiteParserImpl::GetNetworkOutputBindingInfo(size_t subgraphId,
                                              const std::string &name) const {
  CHECK_SUBGRAPH(m_Model, subgraphId);
  ...
  for (unsigned int i = 0; i < outputs.size(); ++i) {
    ...
    if (output.second->name == name) {
      ...
      std::vector<unsigned int> shape =
          m_OverridenOutputShapes.size() > 0
              ? m_OverridenOutputShapes[i]
              : AsUnsignedVector(output.second->shape);
...
std::vector<std::string>
TfLiteParserImpl::GetSubgraphInputTensorNames(size_t subgraphId) const {
  CHECK_SUBGRAPH(m_Model, subgraphId);
  auto inputs = GetSubgraphInputs(m_Model, subgraphId);
...
std::vector<std::string>
TfLiteParserImpl::GetSubgraphOutputTensorNames(size_t subgraphId) const {
  CHECK_SUBGRAPH(m_Model, subgraphId);
  auto outputs = GetSubgraphOutputs(m_Model, subgraphId);
  std::vector<std::string> result;
...
TfLiteParserImpl::SupportedDataStorage::SupportedDataStorage(
    std::unique_ptr<float[]> &&data)
    : m_FloatData(std::move(data)), m_Uint8Data(nullptr), m_Int8Data(nullptr),
      m_Int32Data(nullptr) {}

TfLiteParserImpl::SupportedDataStorage::SupportedDataStorage(
    std::unique_ptr<uint8_t[]> &&data)
    : m_FloatData(nullptr), m_Uint8Data(std::move(data)), m_Int8Data(nullptr),
      m_Int32Data(nullptr) {}

TfLiteParserImpl::SupportedDataStorage::SupportedDataStorage(
    std::unique_ptr<int8_t[]> &&data)
    : m_FloatData(nullptr), m_Uint8Data(nullptr), m_Int8Data(std::move(data)),
      m_Int32Data(nullptr) {}

TfLiteParserImpl::SupportedDataStorage::SupportedDataStorage(
    std::unique_ptr<int32_t[]> &&data)
    : m_FloatData(nullptr), m_Uint8Data(nullptr), m_Int8Data(nullptr),
